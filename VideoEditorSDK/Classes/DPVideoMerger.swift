...
class DPVideoMerger: NSObject {
    func mergeVideos(withFileURLs videoFileURLs: [URL],outputURL:URL?,mediaType:AVFileType, completion: @escaping (_ mergedVideoURL: URL?, _ error: Error?) -> Void) {
        ...
        if isError == false {
            ...
            exportSession?.outputURL = outputURL ?? URL(fileURLWithPath: strFilePath)
